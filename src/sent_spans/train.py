...
def get_sent_boundaries(doc_datasets: dict[int, pd.DataFrame]) -> dict[int, list[int]]:
    ...
    try:
        ...
        for doc_id, spacy_doc in tqdm(
            zip(ids, nlp.pipe(texts, n_process=1, batch_size=10)), total=len(unique_texts), desc="Splitting sentences"
        ):
...
def finetune_fold(
    ...
    callback_doc_df = (
        test_doc_df.groupby("label").apply(lambda x: x.sample(50) if x.shape[0] >= 50 else x).sample(frac=1)
    )  # shuffles
...
def upload_results(upload_key, case_id, sent_pred_df, doc_pred_df):
    ...
    for df, key in [
        ...
        s3_client.upload_fileobj(
            buffer, RESULTS_S3_BUCKET, key, ExtraArgs={"Metadata": metadata, "ContentType": "application/x-binary"}
        )

    # Files necessary to use models in production (LoRA specific; skipping the base model)
    for filepath, key, content_type in [
        (
            best_model_loc(case_id) / "adapter_model.bin",
            f"{upload_key}/{case_id}/adapter_model.bin",
            "application/x-binary",
        ),
        (
            best_model_loc(case_id) / "adapter_config.json",
            f"{upload_key}/{case_id}/adapter_config.json",
            "application/json",
        ),
    ]:
        logger.info(f"Uploading to s3://{RESULTS_S3_BUCKET}/{key}")
